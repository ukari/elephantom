...
defaultPipelineCreateInfo :: SomeStruct GraphicsPipelineCreateInfo
defaultPipelineCreateInfo = SomeStruct $ zero
  { stages = []
...
makePipelineCreateInfo :: RenderPass -> ShaderResource -> PipelineLayout -> SomeStruct GraphicsPipelineCreateInfo
makePipelineCreateInfo renderPass ShaderResource { .. } pipelineLayout = SomeStruct $ zero
  { stages = shaderStages
...
createPipelineResource device renderPass shaderRes@ShaderResource {..} = do
  ...
  (_result, pipelines) <- Vulkan.createGraphicsPipelines device zero
    [ makePipelineCreateInfo renderPass shaderRes pipelineLayout
    ] Nothing
