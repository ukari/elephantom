...
{-# DEPRECATED fontDirectionHint "deprecated(Set to 2) in opentype spec" #-}
data Head = Head
  { majorVersion :: UINT16 -- ^ uint16 majorVersion Major version number of the font header table — set to 1.
  , minorVersion :: UINT16 -- ^ uint16 minorVersion Minor version number of the font header table — set to 0.
  , fontRevision :: FIXED -- ^ Fixed fontRevision Set by font manufacturer.
  , checksumAdjustment :: UINT32 -- ^ uint32 checksumAdjustment
  ...
  , magicNumber :: UINT32 -- ^ uint32 magicNumber Set to 0x5F0F3CF5.
  , flags :: UINT16 -- ^ uint16 flags Bit 0: Baseline for font at y=0.
  ...
  , unitsPerEm :: UINT16 -- ^ uint16 unitsPerEm Set to a value from 16 to 16384. Any value in this range is valid. In fonts that have TrueType outlines, a power of 2 is recommended as this allows performance optimizations in some rasterizers.
  , created :: LONGDATETIME -- ^ LONGDATETIME created Number of seconds since 12:00 midnight that started January 1st 1904 in GMT/UTC time zone.
  , modified :: LONGDATETIME -- ^ LONGDATETIME modified Number of seconds since 12:00 midnight that started January 1st 1904 in GMT/UTC time zone.
  , xMin :: INT16 -- ^ int16 xMin For all glyph bounding boxes. (Glyphs without contours are ignored.)
  , yMin :: INT16 -- ^ int16 yMin For all glyph bounding boxes. (Glyphs without contours are ignored.)
  , xMax :: INT16 -- ^ int16 xMax For all glyph bounding boxes. (Glyphs without contours are ignored.)
  , yMax :: INT16 -- ^ int16 yMax For all glyph bounding boxes. (Glyphs without contours are ignored.)
  , macStyle :: UINT16 -- ^ UINT16 macStyle
  ...
  , lowestRecPPEM :: UINT16 -- ^ uint16 lowestRecPPEM Smallest readable size in pixels.
  , fontDirectionHint :: INT16 -- ^ int16 fontDirectionHint
  -- Deprecated (Set to 2).
  ...
  , indexToLocFormat :: INT16 -- ^ int16 indexToLocFormat 0 for short offsets (Offset16), 1 for long (Offset32).
  , glyphDataFormat :: INT16 -- ^ int16 glyphDataFormat 0 for current format.
  } deriving (Show)
